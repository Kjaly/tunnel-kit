...
> - **Уже работает, хочешь больше** → **[optional-enhancements.md](optional-enhancements.md)** (подписка, мониторинг, алерты, учёт трафика, Telegram-бот, перенос на роутер — по желанию).
>
...
| `optional-enhancements.md` | необязательные апгрейды: подписка, мониторинг, алерты, учёт трафика, бот, роутер |
| `scripts/` | готовые скрипты: health-check, алерты, бэкап, учёт трафика, Telegram-бот, конвертеры для роутера |
