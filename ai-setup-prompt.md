...
| Совсем нет доступа по SSH после настройки | Заходи по ключу `~/.ssh/xray_key` с той же машины. Если лок-аут (закрыт порт/сломан sshd) — открой **веб-консоль/VNC** в панели хостера (в обход SSH) и почини: `ufw allow 22/tcp`, откати плохой drop-in, либо восстановись из snapshot. |
